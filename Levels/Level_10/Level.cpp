...
class Level{
    ...
    public:
        ...
        int getInt(int x, int y, int len){
            string str = "";
            for(int i = 0; i < len; i++){
                str += chr(x + i, y);
            }
        ...
        void setInt(int num, int x, int y, int len){
...
int main(int argc, char *argv[], char *envp[])
{
    ...
    int t = L.getInt(30, 184, 3);
    L.setInt(t - 1, 30, 184, 3);
    if(t < 100) for(int i = 0; i < 188; i++) L.chr(4, i) = '#';
    int v = L.getInt(7, 184, 11);
    ////////////////////////////////
    // 10                  //>160<//
    //                     /////////
    /////////////////////////
    L.setInt(v + 1, 7, 184, 11);
    printLevel(L, FILENAME);
    if(v == 11111) levelComplete();
    return 0;
}
