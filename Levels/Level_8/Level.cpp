...
class AB {
    ...
    private:
        int a;
    public:
        void setA(int x) { a = x - (x & 1); };
        AB(int x) : b(x) { setA(x); };
        bool isAOdd() { return a & 1; };
        bool isBOdd() { return b & 1; };
};

int main(int argc, char *argv[], char *envp[])
{
    // Everything except / is moveable
...
//*
    obj.a = 21;
    int abc = 1 - 1 + 1 - 1 + 1 - 1 + 1 - 1;

// Character 47 is a slash
for(int x = 0; x < 66; x++) L.chr(x, 189) = 47;


//*///////////////////////////////////////////////////////////////
////// Only allow the player to move if obj.b is odd
    if((argc > 1) && obj.isBOdd())
        doMovement(L, Player, Moveable, argv[1]);
    // Output to the console and to the .cpp file
    printLevel(L, FILENAME);
    // Check if obj.a is odd
    if(obj.isAOdd()) levelComplete();
    return 0;
}
